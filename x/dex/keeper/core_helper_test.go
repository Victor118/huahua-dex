...
func (s *CoreHelpersTestSuite) SetupTest() {
	...
	ctx := app.(*chihuahuaapp.App).BaseApp.NewContext(false, tmproto.Header{})

	accAlice := app.(*chihuahuaapp.App).AccountKeeper.NewAccountWithAddress(ctx, s.alice)
	app.(*chihuahuaapp.App).AccountKeeper.SetAccount(ctx, accAlice)
	accBob := app.(*chihuahuaapp.App).AccountKeeper.NewAccountWithAddress(ctx, s.bob)
	app.(*chihuahuaapp.App).AccountKeeper.SetAccount(ctx, accBob)
	accCarol := app.(*chihuahuaapp.App).AccountKeeper.NewAccountWithAddress(ctx, s.carol)
	app.(*chihuahuaapp.App).AccountKeeper.SetAccount(ctx, accCarol)
	accDan := app.(*chihuahuaapp.App).AccountKeeper.NewAccountWithAddress(ctx, s.dan)
	app.(*chihuahuaapp.App).AccountKeeper.SetAccount(ctx, accDan)

	s.app = app.(*chihuahuaapp.App)
	s.ctx = ctx
	s.alice = []byte("alice")
	s.bob = []byte("bob")
	s.carol = []byte("carol")
